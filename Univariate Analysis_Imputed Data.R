...
## Script name: Updated Univariate Analysis_Imputed Data.R
## Script purpose: Conducting statistical analysis using logistic models by cohort - clpsd partitions, only adjusted model PERMANOVA vars
## Date: May 2023
...
pkgs <- c(
  ...
  "webshot2", "nnet", "finalfit", "DescTools", "car", "broom", "generalhoslem", "gofcat", "mice", "VIM"
)
...
jpeg(filename = "Missing data pattern.png", width = 480*2, height = 480, quality = 500)
md.pattern(all_data, rotate.names=TRUE)
dev.off()

jpeg(filename = "MICE plot.png", width = 480*2, height = 400, quality = 200)
mice_plot <- aggr(all_data, col=c('navyblue','yellow'),
                  numbers=TRUE, sortVars=TRUE,
                  labels=names(all_data), cex.axis=.8,
                  gap=3, ylab=c("Missing data","Pattern"))
dev.off()

# over 75% values without missing data

# Check if missing at random (MAR)
mar_check <- all_data %>%
  dplyr::select(-AbxEver) %>%
  mutate(
    white_mar = ifelse(is.na(white),1,0),
    white_mar = as.factor(white_mar),
    mom_mar = ifelse(is.na(Momage),1,0),
    mom_mar = as.factor(mom_mar),
    educ_mar = ifelse(is.na(educ),1,0),
    educ_mar = as.factor(educ_mar),
    Parity_mar = ifelse(is.na(Parity_cat),1,0),
    Parity_mar = as.factor(Parity_cat)
  ) 
emory_mar <- emory %>%
  mutate(
    BMI_mar = ifelse(is.na(BMI_cat),1,0),
    BMI_mar = as.factor(BMI_cat)
  )
mar_check2 <- all_data %>%
  filter(cohort!="Wisconsin") %>%
  mutate(
    abx_mar = ifelse(is.na(AbxEver),1,0)
  )

# 5 missing data for white (4 Wisconsin, 1 MARCH)
# 2 missing data for Momage (1 MARCH, 1 Emory)
# 2 missing data for educ (1 Wisconsin, 1 MARCH)
# 1 missing data for AbxEver - MARCH+Emory (1 Emory)
# No missing data for BMI_cat - Emory 
# 1 missing datum for Parity_cat (1 MARCH)
...
race_ma <- multinom(CST~white+Momage,
                    ...
                    na.action=na.exclude) 
summary(race_ma)
# colSums(model.matrix(race_ptb_ma))
